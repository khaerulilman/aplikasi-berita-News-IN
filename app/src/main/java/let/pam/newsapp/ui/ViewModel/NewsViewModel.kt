...
class NewsViewModel(
    ...
    fun saveUserProfile(newFullName: String, newPassword: String, newDisplayUsername: String, callback: (Boolean) -> Unit) {
        viewModelScope.launch {
            ...
            if (success) {
                _userProfile.postValue(User(newDisplayUsername, newFullName, newPassword))
            }
            callback(success)
        }
    }
